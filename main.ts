...
(async () => {                         // Asynchronous IIFE (Immediately Invoked Function Expression) to handle user input

    while (condition) {                                      // Loop until 'condition' becomes false
        const action = await inquirer.prompt({               // Prompting user to select an action from the list
            name: "action",
            type: "list",
        ...
        if (action.action === "Add") {                        // Adding a todo item
            const todoQuestions = await inquirer.prompt([     // Prompting user to enter the todo item

                {
                    name: "todo",
                    type: "input",
                    message: "Enter the todo item:",
                    validate: function(input) {                // Custom validation function
        ...
        if (action.action === "Update") {                         // Updating a todo item
            const updateIndex = await inquirer.prompt({           // Prompting user to enter the index of the todo to update
                name: "index",
                type: "input",
                message: "Enter the index of the todo you want to update:"
            });
            const updatedTodo = await inquirer.prompt({            // Prompting user to enter the updated todo item
                name: "todo",
                type: "input",
        ...
        if (action.action === "Delete") {                           // Deleting a todo item
            const deleteIndex = await inquirer.prompt({            // Prompting user to enter the index of the todo to delete
                name: "index",
                type: "input",
